...
@app.route('/')
def home():
    medicine_list = df['Medicine'].unique()
    return render_template('index.html', medicine_list=medicine_list)

@app.route('/predict', methods=['POST'])
def predict():
    medicine_name = request.form['medicine_name']
    forecast_months = int(request.form['forecast_duration'])
    ...
    try:
        ...
        future_dates = pd.date_range(start=med_data['Year-Month'].max(), periods=forecast_months + 1, freq='MS')[1:]
        ...
        future_sales_sarima = sarima_fit.forecast(steps=forecast_months)
        ...
        predicted_expiry = max(expiry_date + pd.DateOffset(years=1), latest_entry['Year-Month'] + pd.DateOffset(years=1))
